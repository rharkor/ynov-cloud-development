...
function playGeigerSoundFile(audioContext: AudioContext, src: string, amplitude: number) {
...
function playGeigerClickSound(audioContext: AudioContext, amplitude: number) {
  ...
  oscillator.frequency.exponentialRampToValueAtTime(220, audioContext.currentTime + duration)
...
const Geiger: FC<{
  ...
  const handleRender = useCallback<ProfilerOnRenderCallback>(
    (_id, phase, actualDuration) => {
      if ((phase === phaseOption || phaseOption === "both") && actualDuration > renderTimeThreshold && audioContext) {
        const amplitude = Math.min(1, (actualDuration - renderTimeThreshold) / (renderTimeThreshold * 2))
